// src/features/system-control/pages/ProfileDetail.tsx
...
export function ProfileDetail() {
  ...
  const handleLiveToggleStatus = async () => {
    dispatch(
      pushLog(`[DATABASE]: Toggling status for ${targetUser.username}...`)
    );
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/profiles/${targetUser.id}/toggle`,
        {
          method: "PATCH",
      ...
      if (!response.ok)
        throw new Error("Server rejected status toggle request");
  ...
  const handleLiveDeleteUser = async () => {
    const confirmDelete = window.confirm(
      `Are you sure you want to permanently delete ${targetUser.username}?`
    );
    if (!confirmDelete) return;

    dispatch(
      pushLog(
        `[DATABASE]: Sending deletion query for ${targetUser.username} to MongoDB...`
      )
    );
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/profiles/${targetUser.id}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (!response.ok)
        throw new Error("Server rejected account deletion request");

      dispatch(
        pushLog(
          `[SUCCESS]: Entry successfully dropped from MongoDB collections.`
        )
      );
      navigate("/login");
    } catch (error) {
      dispatch(
        pushLog(`[ERROR]: Failed to delete document entry from database.`)
      );
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <UserProfileCard.StatusRow />
        <UserProfileCard.Title />
        <UserProfileCard.Role />
        <UserProfileCard.Stats />
        ...
        <UserProfileCard.Actions>
          ...
          <button
            onClick={handleLiveToggleStatus}
            style={{
              padding: "5px",
              cursor: "pointer",
